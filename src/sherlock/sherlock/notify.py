...
results of queries.
"""
from src.sherlock.sherlock.result import QueryStatus
...
class QueryNotifyQueue(QueryNotify):
    def __init__(self, result_queue, result=None, verbose=False, print_all=False):
        '''
        result_queue -- multithread safe queue to save reuslt
        result -- Object of type QueryReuslt() containing result for this query
        verbose -- boolean indicating wheather to put verbose output to queue
        output_all -- boolean indicating whether to only put all site, including not found to queue
        '''
        super().__init__(result)
        self.verbose = verbose
        self.output_all = print_all
    ...
    def update(self, result):
        ...
        # only save claimed website for now
        if result.status == QueryStatus.CLAIMED:
            self.result_queue.put((self.result.site_name, self.result.site_url_user))
        return
...
class QueryNotifyPrint(QueryNotify):
    ...
    def update(self, result):
        ...
        if result.status == QueryStatus.CLAIMED:
            self.countResults()
            # put result into queue for asyncronize access
            self.result_queue.put(
                (self.result.site_name, self.result.site_url_user))
    ...
    def finish(self, message="The processing has been finished."):
        ...
        # put finished at end to indicate finished
